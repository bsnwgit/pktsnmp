#!/bin/bash
# pktSNMP uninstall script — Ubuntu Server 22.04/24.04 LTS
...
# credential encryption key), pktsnmp.db and its -wal/-shm, logs, backups and
...
# WorkingDirectory, then $PKTSNMP_INSTALL_DIR, then /opt/pktsnmp.

set -euo pipefail

APP="pktsnmp"
DISPLAY="pktSNMP"
UNIT_PATH="/etc/systemd/system/pktsnmp.service"
...
# migration — plain *.db does not match "pktsnmp.db.pre-upgrade".
...
if [ "$PURGE" -eq 1 ]; then
    ...
    for entry in config.yaml pktsnmp.db logs backups ssl snmp.duckdb; do
        if [ -e "$INSTALL_DIR/$entry" ]; then
            echo "    $entry"
        fi
    done
...
if [ "$PURGE" -eq 1 ] && command -v clickhouse-client &>/dev/null; then
    if clickhouse-client --query "EXISTS DATABASE pktsnmp" 2>/dev/null | grep -q '^1$'; then
        echo ""
        echo "  ClickHouse database 'pktsnmp' still exists and was NOT dropped."
        echo "  To remove it as well:"
        echo "      clickhouse-client --query \"DROP DATABASE pktsnmp\""
    fi
fi
