...
Collector host : COLLECTOR-2-IP (ssh-user, corporate_infrastructure.pem)
pktSNMP host   : SERVER-IP  (ssh-user, your-key.pem)
Collector id   : 3
SNMP pipelines : metrics/firewall

Steps:
  1. Generate a random API token.
  2. SSH to O2 (SERVER-IP) and write the token into pktsnmp.db.
...
O2_HOST         = "SERVER-IP"
O2_USER         = "ssh-user"
O2_KEY_PATH     = r"C:\Users\USER\.ssh\your-key.pem"

COLLECTOR_HOST  = "COLLECTOR-2-IP"
COLLECTOR_USER  = "ssh-user"
COLLECTOR_KEY_PATH = r"C:\Users\USER\.ssh\corporate_infrastructure.pem"
...
PKTSNMP_INGEST  = "http://SERVER-IP:8767/api/snmp/ingest/otlp"
...
def main():
    ...
    try:
        ...
        # Copy to /tmp first so ssh-user can read it
