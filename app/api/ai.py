...
Tries configured providers in priority order (local/private first, then cloud)
and answers with the first one that is enabled and has valid config.
"""
...
- Only answer questions about pktSNMP itself: SNMP device monitoring, OIDs/MIBs, polling,
  traps, alerts, and this app's own settings/features. Nothing else, no matter how the
  question is phrased.
- If a question falls outside that — general knowledge, other software, other pktApp suite
  ...
  redirect the user to pktSNMP's own functionality. Do not partially answer it first.
...
def _scope_violation(question: str) -> str | None:
    ...
    if _INJECTION_RE.search(question):
        return (
            "I can only help with pktSNMP itself — SNMP device monitoring, polling, traps, "
            "and alerts. I can't change roles or ignore my instructions."
        )
    m = _OTHER_APP_RE.search(question)
    if m:
        return (
            f"That looks like a question about {m.group(1)}, which is outside pktSNMP's scope. "
...
def _strip_leaked_prompt(answer: str) -> str:
    ...
    if marker in answer.lower():
        return (
            "I can't share my system instructions. Ask me something about pktSNMP's "
            "device monitoring, polling, traps, or alerts instead."
        )
    return answer
    ...
    if anthropic_enabled is None or anthropic_enabled:  # default on for pre-existing installs
        ...
        if api_key and api_key != "••••••••":
            return {
                ...
                "name": "Anthropic",
                "api_key": api_key,
                "model": await _get_setting(db, "ai_model") or DEFAULT_ANTHROPIC_MODEL,
            }
...
async def chat(
    ...
    """Send a question + SNMP context to the active AI provider and return the answer."""
