"""
/api/integrations/* — named connections to sibling pkt* apps pktsnmp could
...
(pktHub, or another sibling app, calling into pktsnmp).
"""
...
_APPS = ("pktipam", "pktflow", "pktlog", "pktpcap", "pktwifi", "pkthub")
...
async def test_integration(integration_id: int, user: AdminUser, db: aiosqlite.Connection = Depends(get_db)):
    ...
    client = SuiteClient(row["base_url"], decrypt_str(row["suite_token"]), suite_user="pktsnmp", suite_role="admin")
