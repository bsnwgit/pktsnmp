...
async def export_rules(_: CurrentUser, db: aiosqlite.Connection = Depends(get_db)):
    ...
    return StreamingResponse(
        ...
        media_type="text/csv",
        headers={"Content-Disposition": 'attachment; filename="pktsnmp-alert-rules.csv"'},
    )
...
async def import_rules_csv(
    ...
    - conditions: a JSON object string, e.g. '{"device_id": 7}' — the exact
      shape depends on rule_type, same as the "conditions" the UI builds.
    ...
    - Rows are always inserted as new rules (no de-dup by name — multiple
      rules can legitimately share a name/type scoped to different devices).
    """
    import csv, io
