...
Each run creates a timestamped snapshot directory under backup_path:
  <backup_path>/backup_2026-06-24_22-00/
    pktsnmp.db
    config.yaml
    snmp_data.csv.gz   (if backup_include_clickhouse is True)
...
log = logging.getLogger("pktsnmp.backup")
_cfg = get_settings()
...
def run_backup_sync(db_path: str, ch_database: str) -> dict:
    ...
    db_src = Path(db_path)
    if db_src.exists():
        shutil.copy2(str(db_src), str(snap_dir / "pktsnmp.db"))
    ...
    if s["backup_include_clickhouse"]:
        ...
        snmp_gz = snap_dir / "snmp_data.csv.gz"
        try:
            ...
            if snmp_gz.exists() and snmp_gz.stat().st_size > 100:
                result["files"].append("snmp_data.csv.gz")
            else:
                snmp_gz.unlink(missing_ok=True)
        except Exception as e:
