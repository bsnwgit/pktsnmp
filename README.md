...
Selecting one of those rows opens pktSNMP's **real page** inside pktHub —
...
authenticated page, but the assistant itself runs on the resonance server, not inside pktSNMP.
...
| `log_forward_app_name` | `pktsnmp` | APP-NAME in the syslog message |
...
This is worth stating plainly because it was not always true: the cleanup
function existed and the setting was exposed, but **nothing ever called it**.
Poll results accumulated indefinitely — on one deployment reaching 129 million
rows and 28 GB before it was noticed. If you are upgrading from a build older
than this, expect the first prune to remove a great deal.
...
Every run is logged including no-op runs, so "ran and deleted nothing" stays
