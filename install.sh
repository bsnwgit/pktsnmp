...
PREV_INSTALL=0
REMOVE_EXISTING=0
UNIT_FILE="/etc/systemd/system/pktsnmp.service"
...
if [ "$PREV_INSTALL" -eq 1 ]; then
    ...
    echo "Found an existing pktSNMP installation at $INSTALL_DIR (version $PREV_VERSION)."
    ...
    elif [ -n "${PKTSNMP_REMOVE_EXISTING:-}" ]; then
        REMOVE_EXISTING="$PKTSNMP_REMOVE_EXISTING"
    elif [ -t 0 ]; then
...
DB_EXISTED=0
[ -f "$INSTALL_DIR/pktsnmp.db" ] && DB_EXISTED=1
